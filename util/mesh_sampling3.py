...
def get_vert_connectivity(mesh_v):
...
def get_vertices_per_edge(mesh_v):
    """Returns an Ex2 array of adjacencies between vertices, where
    each element in the array is a vertex index. Each edge is included
    only once. If output of get_faces_per_edge is provided, this is used to
    avoid call to get_vert_connectivity()"""

    vc = sp.coo_matrix(get_vert_connectivity(mesh_v))
...
def qslim_decimator_transformer(mesh_v, factor=None, n_verts_desired=None):
...
def generate_transform_matrices(mesh_v, factors):
    ...
    M, A, D, U, F = [], [], [], [], []
    A.append(get_vert_connectivity(mesh_v).astype('float32'))  # A[0]
    M.append(mesh_v)  # M[0]
